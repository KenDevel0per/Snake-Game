  ...
  var game = {
    ...
    loop: function () {
      ...
      if(!stopped) {
        ...
        ctx.fillText("(esc) - reset", 16, 363);
        ctx.fillText("(space) - pause", 16, 381);
      }
      ctx.fillStyle = "lime";
      ctx.fillRect(trail[0].x * gridSize+1, trail[0].y * gridSize+1, gridSize-2, gridSize-2);
      ctx.fillStyle = "lime";
      ...
      if(stopped) {
        ctx.fillStyle = '#ECF0F1';'rgba(250,250,250,250)';
        ctx.font = "small-caps bold 14px Helvetica";
        ctx.fillText("Press arrow keys to start…", 18, 381);
      }
      ctx.fillStyle = '#ECF0F1';
      ctx.font = "bold small-caps 14px Helvetica";
      ctx.fillText("points: " + points, 322, 30);
      ctx.fillText("top: " + pointsMax, 322, 48);
      return reward;
    }
  }
